...
## Prisma implementation (LTI-11)

PostgreSQL persistence for the **Candidate** aggregate is defined in `backend/prisma/schema.prisma` and applied via Prisma Migrate.

| Conceptual entity | SQL table (`@@map`) | Notes |
|-------------------|---------------------|--------|
| Candidate | `candidates` | `email` is unique; `createdAt` / `updatedAt` are stored on the row |
| Education | `educations` | FK `candidateId` → `candidates.id`, `onDelete: Cascade` |
| WorkExperience | `work_experiences` | Same FK/cascade pattern |
| Resume | `resumes` | Includes `uploadDate` with DB default `now()` on insert |

The **`Application`** model (and FKs to `Position`, `InterviewStep`) is **not** in Prisma yet; the `applications` relation on `Candidate` in this document remains conceptual until those entities exist in the schema. A follow-up ticket will add `Application` and wire relations without breaking referential integrity.

**Automated verification:** Integration tests live in `backend/src/tests/candidate.prisma.integration.test.ts`. Run `npm run test:integration` from `backend/` (sets `RUN_DB_INTEGRATION=true` via `cross-env`). Default `npm test` skips them so environments without PostgreSQL still pass CI.
